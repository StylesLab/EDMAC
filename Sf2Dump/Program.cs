...
{
    Console.WriteLine("Usage: Sf2Dump <folder>");
    Console.WriteLine("  Recursively processes all .sf2 files found under <folder>,");
    Console.WriteLine("  writes [name].json for each into the root of <folder>,");
    Console.WriteLine("  and writes samples.txt with the full paths of every .wav found.");
    return;
}
...
var sf2Files = Directory.GetFiles(folder, "*.sf2", searchOptions);
Console.WriteLine($"Found {sf2Files.Length} .sf2 file(s) in '{folder}'");

foreach (var sf2Path in sf2Files)
{
    Console.Write($"  Processing: {sf2Path} ... ");
    try
    {
        var metadata = Sf2Reader.Read(sf2Path);
        var json = JsonSerializer.Serialize(metadata, jsonOptions);
        var outName = Path.GetFileNameWithoutExtension(sf2Path) + ".json";
...
var wavFiles = Directory.GetFiles(folder, "*.wav", searchOptions);
var samplesPath = Path.Combine(folder, "samples.txt");
File.WriteAllLines(samplesPath, wavFiles.Select(Path.GetFullPath));
Console.WriteLine($"Written {wavFiles.Length} sample path(s) to '{samplesPath}'"  );
